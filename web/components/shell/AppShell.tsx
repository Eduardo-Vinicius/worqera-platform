...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-[var(--wq-paper)] text-[var(--wq-text)]">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          />
          <div className="absolute inset-y-0 left-0 flex w-[min(280px,88vw)] shadow-2xl">
            <AppSidebar mobile onNavigate={() => setMobileOpen(false)} />
          </div>
          <button
            type="button"
            className="absolute right-3 top-3 rounded-xl bg-[var(--wq-surface)] p-2 shadow border border-[var(--wq-border)]"
            onClick={() => setMobileOpen(false)}
            aria-label="Fechar"
          >
            <X className="h-5 w-5" />
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="sticky top-0 z-30 md:static">
          <div className="flex items-center gap-3 border-b border-[var(--wq-border)] bg-[var(--wq-paper)] px-4 py-3 md:hidden">
            <button
              type="button"
              className="rounded-xl border border-[var(--wq-border)] bg-[var(--wq-surface)] p-2"
