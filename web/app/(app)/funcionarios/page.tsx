...
export default function FuncionariosPage() {
  ...
  const [setorOptions, setSetorOptions] = useState<Array<{ id: string; label: string }>>([])
  const [setorNames, setSetorNames] = useState<Record<string, string>>({})
  ...
  const loadFuncionarios = async () => {
    try {
      setLoading(true)
      const [list, sectorsRes] = await Promise.all([
        listFuncionariosService(),
        listSectorsV1().catch(() => ({ sectors: [] as any[] })),
      ])
      setFuncionarios(list || [])
      const sectors = (sectorsRes.sectors || [])
        .filter((s: any) => s.active !== false)
        .sort((a: any, b: any) => (a.order || 0) - (b.order || 0))
      const options = sectors.map((s: any) => ({
        id: String(s._id || s.id),
        label: s.name || "Setor",
      }))
      setSetorOptions(options)
      const names: Record<string, string> = {}
      for (const o of options) names[o.id] = o.label
      setSetorNames(names)
    } catch (err: any) {
  ...
  return (
    <div className="-mx-5 -mt-6 md:-mx-8 md:-mt-7">
      <AppHeader
        title="Funcionários"
        subtitle="Cadastre e associe por setor"
        actions={
          <Button
            size="sm"
      ...
      <div className="mx-auto max-w-[1100px] space-y-6 px-5 py-6 md:px-8">
                      ...
                      <tr key={func.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-4 py-3">
                          <Badge variant="outline" className="bg-slate-50 text-slate-700 border-slate-200">
                            {setorNames[func.setorId] || func.setorId}
                          </Badge>
                        </td>
